...
{
	adcInit(adc, port, pin);// Initialize the ADC: adc_init(ADC1 or ADC2, Port, Pin)
	if(adc == 1)
	{
...
void adcMultiInit(char adc, char channels, char * adc_channels)
{
	...
	for(i=0;i< channels;i++)
	{
		if(adc_channels[i]<8)
		{
			InitGPIO(PA,adc_channels[i],IN,I_A);
		}
		else if(adc_channels[i]<10)
		{
			InitGPIO(PB,adc_channels[i]-8,IN,I_A);
		}
		else if(adc_channels[i]<16)
		{
			InitGPIO(PC,adc_channels[i]-10,IN,I_A);
		}
	}
	if(adc == 1)
	{
	...
	while(1)
		{
			...
			{
				temp_rx = ADC1->DR;
analog_rx[i] = pow(1.02,((temp_rx-300)));
//    analog_rx[i] = (temp_rx);
				i++;
				if(i == channels)
				{
